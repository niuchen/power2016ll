...
public class Lunar implements Serializable {
	...
	public String toString() {
		return getYear() + "(" + LunarUtil.cyclical(getYear()) + ")年" + (isLeap() ? "闰" : "") + LunarUtil.getChnMonth(getMonth()) + "月"
		        + LunarUtil.getChnDay(getDay());
	}
	...
	public String toString(boolean outputYear, boolean outputMonth, boolean outputDay) {
		...
		if (outputMonth) {
			sb.append(isLeap() ? "闰" : "").append(LunarUtil.getChnMonth(getMonth())).append("月");
		}
		if (outputDay) {
