...
public final class LunarUtil {
	...
	private static final long[] LUNAR_INFO = new long[] {
	        ...
	        0x0d520 };
	//@formatter:off
	private static String[] runYue=new String[]{"20124","20204"};
	public static Optional<Solar> lunar2Solar(int year, int month, int day) {
		String yue = year+""+month;
		return lunar2Solar(year,month,day,Arrays.asList(runYue).contains(yue));
	}
	/**
	 * 陰历转陽历
	 * 
	 * 輸入的年-1後，循環遍历獲取該年天数，一直遞減到1900年的己亥年，求出年的總天数。
	 * 
	 * 再求出該年的陰历月日到春節一月一日的天数（這裏就要註意闰月了）。
	 * 
	 * 如果传入的月数大於闰月，則要加上闰月的天数。
	 * 
	 * 如果传入的是闰月，則沒關系，直接加上前面幾個月的天数和自己的日子。
	 * 
	 * 最後得到該陰历日期到1900年1月31日的天数，再換算出公历Date.
	 *
	 * @param year
	 *            the year
	 * @param month
	...
	public static Optional<Solar> lunar2Solar(int year, int month, int day, boolean isLeapMonth) {
		...
		// 【1】数據準備
		int totalDays = 0;// 該陰历到公历1900年1月31日的總日子数

		// 【2】求出該年到春節一月一日的總天数
		// 2.1.獲取該年的闰月
		int leapMonth = leapMonth(year);
		// 2.2.如果該年存在闰月，且传入的月份在闰月後，先加上一個闰月日子数
		if (leapMonth > 0 && leapMonth < month) {
			totalDays += leapDays(year);
		}
		// 2.3.如果传入的是闰月，則先加上一個該月非闰月的天数，因為下面直接就跳過該月了。
		else if (leapMonth == month && isLeapMonth) {
			totalDays += monthDays(year, month);
		}
		// 2.4.求出到除本月後，其他先前月份的總日子数
		...
		// 【3】求出到1900年的己亥年，年的總天数。
		for (--year; year >= 1900; year--) {
			totalDays += yearDays(year);
		}

		// 【4】計算從1900年1月30日到入參陰历的相對時間，以此得出該陰历的陽历日期
	...
	public static Optional<Lunar> solar2Lunar(int year, int month, int day) {
		...
		/*
		 * 求出了农历年份。 农历年份是以幹支標識的，以1900年1月31日己亥年開始。 默認1900為己亥年，1901則為辛醜。
		 * 但這個1901與公历的1901不同，如公历1901年1月還為己亥年，則其陰历年為1900。
		 */
		lunar.setYear(iYear);

		// 3.2.計算农历月份。包括是否為闰月
		leapMonth = leapMonth(iYear); // 闰哪個月,1-12
		boolean leap = false;

		// 用當年的天数offset,逐個減去每月（农历）的天数，求出當天是本月的第幾天
	...
	/**
	 * * 传回农历 y年的總天数 * * @param yyyy 四位年数 * @return.
	 *
	 * @param yyyy
	 *            the yyyy
	 * @return the int
