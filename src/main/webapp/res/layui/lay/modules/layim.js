...
//        ,'<li class="layui-icon layim-tool-skin" layim-event="skin" title="换肤">&#xe61b;</li>'
        ,'{{# if(d.base.find){ }}'
        ,'<li class="layui-icon layim-tool-find" layim-event="find" title="添加群">&#xe61f;</li>'
        , '{{# } }}'
         , '{{# if(d.base.apply){ }}'
...
//          ,'<span class="layui-icon layim-tool-image layim-pz-images" title="上传图片" layim-event="image">&#xe60d;<input type="file" name="file"></span>'
          , '<span class="layui-icon layim-tool-image layim-pz-images" title="上传图片" layim-event="image"><i>&#xe60d;</i><img src="res/layui/css/modules/layer/default/loading-2.gif" width="20" height="20" style="display:none;"/><input type="file" name="file"></span>'
          ,'{{# if(d.base && d.base.uploadFile){ }}'
          ,'<span class="layui-icon layim-tool-image layim-pz-file" title="发送文件" layim-event="image" data-type="file"><i>&#xe61d;</i><img src="res/layui/css/modules/layer/default/loading-2.gif" width="20" height="20" style="display:none;"/><input type="file" name="file"></span>'
//          ,'<span class="layui-icon layim-tool-image layim-pz-file" title="发送文件" layim-event="image" data-type="file">&#xe61d;<input type="file" name="file"></span>'
           ,'{{# }; }}'
    ...
    var post = function(options, callback, tips){
        ...
        return $.ajax({
          ...
          ,success: function(res){
              ...
              : res.msg == 'unauthorized' ? (function () { location.href = '/login?msg=' + res.msg; })() : layer.msg(res.msg || ((tips || 'Error') + ': LAYIM_NOT_GET_DATA'), {
                  time: 5000
              });
...
//              console.log('需要加载历史记录哦');
              if (!ul.prev().hasClass('layim-chat-system')) {
                  ...
                  ul.before('<div class="layim-chat-system"><img style="margin-top:15px;" src="res/layui/css/modules/layer/default/loading-2.gif" /</div>');
              }
...
//        console.log('打开上传文件窗口。。。。');
        layui.upload({
          ...
          , success: function (res) {
              ...
              if (res.code == 0) {
                  if (type === 'images') {
                      focusInsert(thatChat.textarea[0], 'img[' + (res.msg || '') + ']');
                  } else if (type === 'file') {
                      focusInsert(thatChat.textarea[0], 'file(' + (res.msg || '') + ')[' + (res.name || '下载文件') + ']');
                  }
              } else {
                  layer.msg(res.msg || '上传失败');
              }
