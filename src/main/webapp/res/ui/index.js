...
$.fn.Tab = function (options) {
    ...
    function addTab(item) {
        ...
        if (tab.find("#" + item.id).length == 0) {
            ...
            if (item.url) {
                var $iframe = $("<iframe class=\"POWERMS_iframe\" id=\"iframe" + item.id + "\" height=\"100%\" width=\"100%\" src=\"" + item.url + "\" frameBorder=\"0\"></iframe>")
...
loadnav = function () {
    ...
    tablist = $("#tab_list_add").Tab({
        items: [
            { id: 'desktop', title: '欢迎首页', closed: false, icon: 'fa fa fa-desktop', url: contentPath + 'home' }
        ],
        tabScroll: false,
...
//                });
        ...
        currentEvent: function (moduleId) {
            ...
            var _cur = $("#iframe"+moduleId)[0];
            var cur = _cur?_cur.contentWindow||_cur:false;
            if(cur && cur.__reportInit){//当前页面回调
            	cur.__reportInit();
        	}
        }
    });
    ...
    $("#GoToHome").click(function() {
		tablist.newTab({id : "desktop",title : "欢迎首页",closed : !1,icon : "fa fa fa-desktop",url : contentPath + "/home"})
	});
    //动态加载导航菜单
    var _html = "";
    var index = 0;
    ...
    $("#nav li.sub").hover(function (e) {
        ...
        if (top + height > wheight) {
            $ul.css('top', parseInt('-' + (height - 11))).css('left', '150px')
        } else {
            $ul.css('top', '0px').css('left', '150px');
        }
    }, function (e) {
        $(this).find('ul').hide();
        ...
        $.each(authorizeMenuData, function (i) {
            ...
            if (row.parentid == moduleId) {
                if (isbelowmenu(row.moduleid) == 0) {
                    ...
                    _html += '<li class="sub" title=' + (row.remark == null ? "" : row.remark) + '><a id=' + row.moduleid + '><i class="fa ' + row.icon + '"></i>' + row.fullname + '</a>';
    ...
    function getchildnav(moduleId) {
        ...
        $.each(authorizeMenuData, function (i) {
            var row = authorizeMenuData[i];
            if (row.parentid == moduleId) {
                _html += '<li title=' + (row.remark == null ? "" : row.remark) + '><a id=' + row.moduleid + '><i class="fa ' + row.icon + '"></i>' + row.fullname + '</a></li>';
                navJson[row.moduleid] = row;
            }
        });
        _html += '</ul></div>';
...
$.removeTab = function (type) {
    var Id = tabiframeId().replace("iframe", "");
    var $tab = $(".tab-div");
