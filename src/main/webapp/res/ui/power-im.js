$.IMSignalR = function (options) {
    var defaults = {
        url: "http://localhost:881/signalr",//服务地址
        userId: "",//用户登录Id
    ...
    chat.client.IMUpdateUserList = function (userAllList, onLineUserList) {
        ...
        $.each(userAllList, function (Id, item) {
            if (Id != options.userId) {
                if (userDepartmentList[item.F_DepartmentId] == undefined) {
                    userDepartmentList[item.F_DepartmentId] = {};
                    userDepartmentList[item.F_DepartmentId].DepartmentName = item.F_DepartmentId;
                    if (item.F_UserOnLine == 1) {
                        userDepartmentList[item.F_DepartmentId].onLineNum = 1;
                    }
                    else {
                        userDepartmentList[item.F_DepartmentId].onLineNum = 0;
                    }
                    userDepartmentList[item.F_DepartmentId].UserList = [];
                    userDepartmentList[item.F_DepartmentId].UserList.push(item);
                }
                else {
                    if (item.F_UserOnLine == 1) {
                        userDepartmentList[item.F_DepartmentId].onLineNum++;
                    }
                    userDepartmentList[item.F_DepartmentId].UserList.push(item);
                }
            }
    ...
    chat.client.IMUpdateUserStatus = function (userId, isOnLine) {
        imMethod.userAllList[userId].F_UserOnLine = isOnLine;
...
//                console.log(item);
                ...
                var userJson = userAllList[i].UserList.sort(function (a, b) { return a.F_UserOnLine < b.F_UserOnLine ? 1 : -1 });
                if (userJson.length > 0) {
                    html += '<ul class="message-chatlist">';
                    $.each(userJson, function (j) {
                        var src = "/Content/images/off-line.png"
                        if (userJson[j].F_UserOnLine == 1) {
                            src = "/Content/images/on-line.png"
                        }
                        //if (userJson[j].HeadIcon) {
                        //    src = userJson[j].HeadIcon;
                        ...
                        html += '<div data-value="' + userJson[j].F_UserId + '" class="message-onename">' + userJson[j].F_RealName + ' <span>[' + userAllList[i].DepartmentName + ']</span></div>';
                        html += '</li>';
                    });
                    html += '</ul>';
                }
