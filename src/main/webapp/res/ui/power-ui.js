...
$.fn.ComboBox = function (options) {
    //options参数：description,height,width,allowSearch,url,param,data,defaultValue：默认显示第一行
    var $select = $(this);
    if (!$select.attr('id')) {
        return false;
    }
        ...
        if (json.length > 0) {
            ...
            $.each(json, function (i) {
                ...
                if (searchValue != undefined) {
                    ...
                    $_html.append('<li data-value="' + row[options.id] + '" title="' + title + '">' + row[options.text] + '</li>');
                    if(options.defaultValue && i==0){
					    var $option = $("#" + $select.attr('id') + "-option");
					    $select.attr('data-value', row[options.id]);
	                    $select.attr('data-text', row[options.text]);
				        $select.find('.ui-select-text').html(row[options.text]).css('color', '#000');
				        $option.find('ul').find('[data-value=' + row[options.id] + ']').addClass('liactive')
                    }
                }
            });
...
$.fn.ComboBoxSetValue = function (value) {//可以设置空值
...
$.fn.ComboBoxTreeSetValue = function (value) {//可以设置空值
    //if (value == "") {
    //    return;
    //}
    var $select = $(this);
...
dialogOpen = function (options) {
    ...
    top.layer.open({
        ...
        btn: options.btn,
        yes: function () {
            options.callBack("__"+options.id)
        }, cancel: function () {
...
;(function ($) {
    $.fn.checkNum = function (obj) {
        ...
        $(this).on('keyup blur',function (e) {
            ...
            if(option.decimalLength==0){
            	entryValue = entryValue.replace(/[^0-9]/g,'');
            	if(entryValue.length> option.integerLength){
            		entryValue = entryValue.substring(0, option.integerLength)
            	}
            	 $(this).val(entryValue);
            	 return ;
            }
